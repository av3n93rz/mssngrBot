    ...
    return {
        ...
        function: async function (messageObj, str) {
            return new Promise(function (resolve, reject) {
                ...
                if (regex[1])
                    post["language"] = regex[1].trim();
                if (regex[2])
                    post["source_code"] = regex[2].trim();
                if (regex[3])
                    post["input"] = regex[3].trim();
                request.post("http://api.paiza.io:80/runners/create", {
                    form: post
                    ...
                    (function loopResponse(id, count) {
                        request.get("http://api.paiza.io/runners/get_details?api_key=guest&id=" +
                            id, function (err, resp, body) {
                            if (err || resp.statusCode != 200) {
                                reject("Failed to run code\n\n" + body);
                                return;
                            }
