...
class PermissionManager {
    ...
    write(file) {
        if (!file && !this.filePath)
            throw Error('No file path specified');
        file = file || this.filePath;
        fs.writeFileSync(file, JSON.stringify(this.perms));
    }
    _touchUser(userID) {
        if (!(userID in this.perms))
            this.perms[userID] = {};
    }
        ...
        return async function (messageObj) {
            ...
            const check = (() => {
                ...
                if (typeof check === 'undefined') {
                    if (!api.commandMap[commandStr]._admin)
                        return true;
                    if (api.admins.indexOf(Number(messageObj.sender)) > -1)
                        return true;
                }
                return false;
            })();
    ...
    const funcs = {
        ...
        check: async function (messageObj, userID) {
            ...
            for (const permission of Object.keys(permissions).sort()) {
                _permissions.push(`${permissions[permission] ? '+' : '-'} ${permission}`);
            }
    ...
    return {
        ...
        onFinishLoad: function () {
            ...
            for (const commandStr in api.commandMap) {
                api.commandMap[commandStr].function = permissionsFunctionWrapper(api.commandMap[commandStr].function);
